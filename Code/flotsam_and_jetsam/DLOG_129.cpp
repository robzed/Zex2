...
static void DoDialogUpdate(DialogPtr dlog)
	{
		...
		FrameDefault(dlog,BUT6_OK,TRUE);
...
static DialogPtr OpenThisDialog()
	{
		...
		PutDlgString(dlog,EDIT5,"\2pEdit Me" /* Or whatever */, TRUE);
...
static int DoDialogItem(DialogPtr dlog, short itemHit)
	{
		...
		switch(type) {
			...
			case statText:
				switch(itemHit) {
					case STXT2_Key:		/* NOT Enabled */
						break;
					case STXT3:		/* NOT Enabled */
						break;
					case STXT4_Your:		/* NOT Enabled */
						break;
					}
				break;
			case editText:
